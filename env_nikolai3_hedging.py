...
class UniswapV3LPGymEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        self.hedger = hedger # type: ignore
    ...
    def step(self, action):
        ...
        # delta-hedging on Binance 
        if self.hedger is not None:
            in_range_now = (
                self.active
                and (price_lower <= p_cex <= price_upper)
            )
            target_delta = -self.x_prev if in_range_now else 0.0
            hedge_pnl = self.hedger.update(ts, target_delta)
            reward += hedge_pnl
