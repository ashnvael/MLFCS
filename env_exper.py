...
class UniswapV3LPGymEnv(gym.Env):
    ...
    def _event_gas_fees(event_name, current_timestamp, num_of_events=20):
        ...
        """
        t0 = time.time()
        print("[_event_gas_fees] Starting...")

        t1 = time.time()
        file_paths = glob.glob(f"{UNISWAP_POOL_EVENTS_PATH}/uniswap_lp_data_*.csv")
        print(f"Loaded {len(file_paths)} files in {time.time() - t1:.4f} seconds.")
        ...
        for path in file_paths:
            ...
            # Step 1: Get first timestamp
            t1 = time.time()
            first_row = pd.read_csv(path, usecols=["timestamp"], nrows=1)
            first_ts = pd.to_datetime(first_row["timestamp"].iloc[0])
            print(f"[{path}] First timestamp read in {time.time() - t1:.4f} sec.")

            # Step 2: Get last timestamp (slow due to `open(path)` line)
            t2 = time.time()
            with open(path) as f:
                n_lines = sum(1 for _ in f)
            last_row = pd.read_csv(path, usecols=["timestamp"],
                                skiprows=lambda i: i != 0 and i != n_lines - 1)
            last_ts = pd.to_datetime(last_row["timestamp"].iloc[-1])
            print(f"[{path}] Last timestamp read in {time.time() - t2:.4f} sec. (lines={n_lines})")

            # Step 3: Check and load data if timestamp in range
            if first_ts <= current_timestamp and last_ts >= current_timestamp:
                ...
                print(f"[{path}] Matching events filtered in {time.time() - t3:.4f} sec.")

            print(f"[{path}] Total file processed in {time.time() - t_file_start:.4f} sec.")
        ...
        if events_data:
            ...
            print("[_event_gas_fees] No matching data found.")
            avg_gas_fee = 0.0
        print(f"Aggregated and filtered in {time.time() - t4:.4f} sec.")

        # Step 5: Read ETH price data
        t5 = time.time()
        ...
        current_eth_price = matched_row["open"].iloc[0] if not matched_row.empty else 0.0
        print(f"ETH price fetched in {time.time() - t5:.4f} sec.")

        total_time = time.time() - t0
        print(f"[_event_gas_fees] Completed in {total_time:.4f} seconds.")
    ...
    def compute_pnl(self, current_timestamp, action):
        def profile(func, *args, **kwargs):
            print(f"[ENTRY] Calling {func.__name__}...")
            start = time.time()
            result = func(*args, **kwargs)
            duration = time.time() - start
            print(f"[DONE] {func.__name__} took {duration:.4f} seconds.")
            return result
        ...
        if previous_action == 0:
            if next_action == 0:
                return 0
            return (
                profile(self._pool_fees) 
                - profile(self._event_gas_fees, "Mint", current_timestamp) 
                - profile(self._hedging_transaction_costs, current_timestamp, self.eth_position_size[-1]) 
                - profile(self._hedging_funding_rate, current_timestamp, self.eth_position_size[-1]) 
                - profile(self._impermanent_loss)
            )
        ...
        if previous_action == 1:
            if next_action == 1:
                return (
                    profile(self._pool_fees)
                    - profile(self._hedging_funding_rate, current_timestamp, self.eth_position_size[-1])
                    - profile(self._hedging_rebalancing_costs, current_timestamp, self.eth_position_size[-1], self.eth_position_size[-2])
                    - profile(self._impermanent_loss)
                )
            return (
                - profile(self._event_gas_fees, "Burn", current_timestamp)
                - profile(self._event_gas_fees, "Collect", current_timestamp)
                - profile(self._hedging_transaction_costs, current_timestamp, self.eth_position_size[-1])
            )
